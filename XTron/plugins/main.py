...
pongg = PING_MSG if PING_MSG else "✘ 𝗝𝗮𝗺𝗲𝘀 𝗕𝗼𝘁 ✘"
RIZ_PIC = ALIVE_PIC if ALIVE_PIC else "https://te.legra.ph/file/62296205396eaeb7f4d1f.jpg"
Alivemsg = ALIVE_MSG if ALIVE_MSG else "𝗝𝗮𝗺𝗲𝘀 𝗕𝗼𝘁 𝗶𝘀 𝗔𝗹𝗶𝘃𝗲...!"
...
**pormspam**: Spams Porn For a given Counter. 
**COMMANDS:**
i) {hl}pormspam <counter>
...
help_menu = f"""
**Welcome To James Bot Help Menu**
...
async def stats(event):
   ...
   if event.sender_id == OWNER_ID or e.sender_id in DEV:
       ...
       # await event.edit("`Processing..`")
        ...
        rizoel += f"✘ **HERE IS YOUR James BOT STATS** ✘\n\n"
        rizoel += f"`Users:`\t**{u}**\n"
        rizoel += f"`Groups:`\t**{g}**\n"
    ...
    if e.sender_id in SUDO_USERS or e.sender_id in DEV:
        ...
        if len(Rizoel) == 2:
            ...
            if re.search(TheRiZoeL.lower(), message.lower()):
                return await e.reply("**Sorry !!** I can't Spam On @itzz_james , Because he is my owner.")
...
async def pspam(e):
    if e.sender_id in SUDO_USERS or e.sender_id in DEV:
        ...
        if len(rizoel) == 1:
            ...
            usage = f"**MODULE NAME : PORM SPAM** \n\n **COMMAND:** `{hl}pormspam <count>`"
            await e.reply(usage, parse_mode=None, link_preview=None )
...
async def dmspam(e):
    ...
    if e.sender_id in SUDO_USERS or e.sender_id in DEV:
        ...
        if len(Rizoelsexy) == 2:
            ...
            g = a.id
            if int(g) in RiZoeLX:
                text = f"I can't Dm To @itzz_james."
                await e.reply(text, parse_mode=None, link_preview=None )
            elif int(g) == OWNER_ID:
                text = f"This guy is Owner Of this Bot."
                await e.reply(text, parse_mode=None, link_preview=None )
            elif int(g) in DEV:
...
async def tspam(e):
   ...
   if e.sender_id in SUDO_USERS or e.sender_id in DEV:
        ...
        if len(RizoelOk) == 2:
            ...
            if re.search(TheRiZoeL.lower(), message.lower()):
                return await e.reply("**Sorry !!** I can't Spam On @itzz_james")
            if int(e.chat_id) in GROUP:
                return await e.reply("Sorry !! I can't spam here")
...
async def raid(e):
    ...
    if e.sender_id in SUDO_USERS or e.sender_id in DEV:
        ...
        if len(Tron) == 2:
            ...
            g = a.id
            if int(g) in RiZoeLX:
                text = f"I can't raid on @itzz_james."
                await e.reply(text, parse_mode=None, link_preview=None )
            elif int(g) == OWNER_ID:
                text = f"This guy is Owner Of this Bot."
                await e.reply(text, parse_mode=None, link_preview=None )
    ...
    if e.sender_id in SUDO_USERS or e.sender_id in DEV:
        ...
        if len(e.text) > 11:
            ...
            if int(user_id) in RiZoeLX:
                text = f" can't raid on @itzz_james."
                await e.reply(text, parse_mode=None, link_preview=None )
